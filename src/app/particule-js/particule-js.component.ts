import { Component, OnInit } from '@angular/core';

@Component({
  selector: 'app-particule-js',
  templateUrl: './particule-js.component.html',
  styleUrls: ['./particule-js.component.scss'],
})
export class ParticuleJsComponent implements OnInit {
  myStyle: object = {};
  myParams: object = {};
  width: number = 100;
  height: number = 100;

  constructor() {}

  ngOnInit() {
    this.myStyle = {
      position: 'fixed',
      width: '100%',
      height: '100%',
      'z-index': -1,
      top: 0,
      left: 0,
      right: 0,
      bottom: 0,
    };

    this.myParams = {
      // particles: {
        particles: {
          number: { value: 1, density: { enable: true, value_area: 800 } },
          color: { value: '#FF0000' },
          shape: {
            type: 'circle',
            stroke: { width: 0, color: '#000000' },
            polygon: { nb_sides: 5 },
            image: { src: 'img/github.svg', width: 100, height: 100 },
          },
          opacity: {
            value: 0.5,
            random: false,
            anim: { enable: false, speed: 1, opacity_min: 0.1, sync: false },
          },
          size: {
            value: 3,
            random: true,
            anim: { enable: false, speed: 40, size_min: 0.1, sync: false },
          },
          line_linked: {
            enable: true,
            distance: 150,
            color: '#00FF00',
            opacity: 0.4,
            width: 1,
          },
          move: {
            enable: true,
            speed: 20,
            direction: 'none',
            random: false,
            straight: false,
            out_mode: 'out',
            bounce: false,
            attract: { enable: false, rotateX: 60, rotateY: 120 },
          },
        },
        interactivity: {
          detect_on: 'canvas',
          events: {
            onhover: { enable: false, mode: 'repulse' },
            onclick: { enable: true, mode: 'push' },
            resize: true,
          },
          modes: {
            grab: { distance: 40, line_linked: { opacity: 1 } },
            bubble: {
              distance: 40,
              size: 4,
              duration: 2,
              opacity: 8,
              speed: 3,
            },
            repulse: { distance: 200, duration: 1 },
            push: { particles_nb: 4 },
            remove: { particles_nb: 200 },
          },
        },
        retina_detect: true,
      // },
      // {
      //   number: {
      //     value: 200,
      //   },
      //   color: {
      //     value: '#ff0000',
      //   },
      //   shape: {
      //     type: 'triangle',
      //   },
      // },
    };
  }
}
